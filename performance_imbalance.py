...
def performance_imbalance_scenario_single_run(bat_path, test_result_file_path, output_store_path):
    """
    单次性能不平衡场景主函数：清理→启动→等待20分钟→第一次测试→第二次测试(期间对随机一半节点进行传输延迟)→结果存储→停止系统
    ...
    try:
        ...
        # -------------------------- 6. 第二次测试：等待30分钟后开始，期间对随机一半节点进行传输延迟 --------------------------
        logging.info("\n【步骤6/6】等待30分钟后开始第二次测试（期间对随机一半节点进行传输延迟操作）...")
        ...
        second_test["test_phase"] = "imbalance"
        second_test["phase_description"] = f"性能不平衡测试（异常状态 - 延迟: {TRANSMISSION_DELAY_MS}ms ±{DELAY_VARIANCE_MS}ms，影响节点: {selected_nodes}）"
        second_test["transmission_delay_ms"] = TRANSMISSION_DELAY_MS
        second_test["delay_variance_ms"] = DELAY_VARIANCE_MS
        second_test["affected_nodes"] = selected_nodes
