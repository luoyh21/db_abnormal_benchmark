...
import os
import shutil
from config import node_num, server_ip, abnormal_scenario, OUTPUT_STORE_PATH, BENCHMARK_CONFIG_PATH, CLIENT_MULTIPLIER
...
def modify_benchmark_config():
    """
    修改benchmark配置文件，通过增加客户端数量实现过载配置
    将DATA_CLIENT_NUMBER和SCHEMA_CLIENT_NUMBER都乘以CLIENT_MULTIPLIER倍数
    将DEVICE_NUMBER乘以CLIENT_MULTIPLIER倍数
    """
    try:
        # 备份原始配置文件
        ...
        modified_lines = []
        for line in lines:
            if line.strip().startswith('DATA_CLIENT_NUMBER'):
                # 获取原始值并乘以倍数
                original_value = int(line.split('=')[1].strip())
                new_value = original_value * CLIENT_MULTIPLIER
                modified_lines.append(f"DATA_CLIENT_NUMBER={new_value}\n")
                logging.info(f"修改DATA_CLIENT_NUMBER: {original_value} -> {new_value} (倍数: {CLIENT_MULTIPLIER})")
            elif line.strip().startswith('SCHEMA_CLIENT_NUMBER'):
                # 获取原始值并乘以倍数
                original_value = int(line.split('=')[1].strip())
                new_value = original_value * CLIENT_MULTIPLIER
                modified_lines.append(f"SCHEMA_CLIENT_NUMBER={new_value}\n")
                logging.info(f"修改SCHEMA_CLIENT_NUMBER: {original_value} -> {new_value} (倍数: {CLIENT_MULTIPLIER})")
            elif line.strip().startswith('DEVICE_NUMBER'):
                original_value = int(line.split('=')[1].strip())
                new_value = original_value * CLIENT_MULTIPLIER
                modified_lines.append(f"DEVICE_NUMBER={new_value}\n")
                logging.info(f"修改DEVICE_NUMBER: {original_value} -> {new_value} (倍数: {CLIENT_MULTIPLIER})")
            else:
        ...
        logging.info("✅ 配置文件修改完成，已应用客户端过载配置")
        return True
...
def over_load_client_scenario(bat_path: str = "test.bat", 
    ...
    logging.info(f"\n{'='*80}")
    logging.info(f"开始单次客户端过载场景实验（客户端倍数: {CLIENT_MULTIPLIER}）")
    logging.info(f"{'='*80}")
    ...
    # 调用客户端过载场景函数
    exp_result = over_load_client_scenario_single_run(
        bat_path=bat_path,
...
def over_load_client_scenario_single_run(bat_path, test_result_file_path, output_store_path):
    """
    单次客户端过载场景主函数：清理→启动→等待20分钟→异常测试(客户端过载配置)→恢复配置→停止系统
    参数：
    ...
    try:
        ...
        logging.info("\n【步骤5/5】等待20分钟后开始异常测试（期间进行客户端数量配置修改操作）...")
        time.sleep(20 * 60)  # 等待20分钟
        
        # 创建异步执行配置修改操作的线程
        def config_modification_operation():
            logging.info("等待10分钟后修改配置文件为客户端过载配置...")
            time.sleep(10 * 60)  # 等待10分钟
            
            logging.info("开始修改配置文件为客户端过载配置...")
        ...
        abnormal_test["phase_description"] = f"客户端过载配置测试（客户端数量倍数: {CLIENT_MULTIPLIER}）"
